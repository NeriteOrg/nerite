...
export function RedeemScreen() {
  ...
  if (!hasUpdatedRedemptionRate.current && redemptionRate.data) {
    if (maxFee.isEmpty) {
      maxFee.setValue(
        fmtnum(
          dn.mul(dnum18(redemptionRate.data), 1),
          "pct2z",
        ),
      );
    }
  ...
  return (
    <Screen
      heading={{
        title: (
          <HFlex>
            Redeem <TokenIcon symbol="USND" /> USND for
            <TokenIcon.Group>
          ...
          field={
            <InputField
              ...
              drawer={amount.isFocused
                ...
                ? {
                  mode: "error",
                  message: `Insufficient USND balance. You have ${fmtnum(boldBalance.data)} USND.`,
                }
                : null}
              label="Redeeming"
              placeholder="0.00"
              secondary={{
                start: `$${
                  ...
                  boldBalance.data && dn.gt(boldBalance.data, 0) && (
                    <TextButton
                      label={`Max ${fmtnum(boldBalance.data)} USND`}
                      onClick={() => {
                        if (boldBalance.data) {
