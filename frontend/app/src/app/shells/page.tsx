...
import { useAllocations } from '@/src/shell-hooks';
...
export default function ShellsPage() {
  ...
  const allocationUsers = allocations?.map((allocation) => getAddress(allocation.user));
  const { data: ensNames } = useEnsNames(allocationUsers);

  const users = useMemo(() => {
    return allocations?.map((allocation, index) => {
      const address = getAddress(allocation.user);
      const activities = allocation.activities.map(activity => activity.label);
      return {
        address,
        ensName: ensNames?.find(ensName => isAddressEqual(ensName.address, address))?.ensName ?? null,
        shellpoints: {
          total: parseInt(formatUnits(allocation.shells.balance, 18)),
          mostRecent: null,
        },
