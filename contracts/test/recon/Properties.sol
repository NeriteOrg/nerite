...
abstract contract Properties is BeforeAfter, Asserts {
    ...
    function property_TR04(uint256 expectedDelta, uint256 debtAfter, uint256 debtBefore, uint256 fee) internal {
        ...
        eq(
            expectedDelta,
            (debtAfter - debtBefore) - fee,
            "TR-04: excluding update/open fee, trove debt delta is the debtIncrease - debtDecrease"
        );
    }

    // NOTE: You should implement this
    function property_TR06(
        uint256 troveDebtBeforeLiquidation,
        uint256 beforeGhostDebtAccumulator,
        uint256 afterGhostDebtAccumulator
    ) internal {
        ...
        eq(
            debtPercentageOfTotal * debtDelta,
            debtPercentageOfTotal * troveDebtBeforeLiquidation,
            "TR-06: Contribution of user collateral should be equal to the percent offered in a liquidation"
        );
    }
    ...
    function property_CS04() public {
        ...
        gte(
            collSurplusPoolBalanceWeth,
            collSurplusPool.getCollBalance(),
            "CS-04: collSurplusPool balance  > getCollBalance"
        );
    }
            ...
            if (boldDebtForDistribution > 0 || collateralForDistribution > 0) {
                ...
                if (boldDebtForDistribution > 0) {
                    gte(
                        ...
                        debt,
                        "TR-03: getLatestTroveData always returns up-to-date, post-accrual debt value"
                    );
                } else if (collateralForDistribution > 0) {
                    gte(
                        troveData.entireColl,
                        coll,
                        "TR-03: getLatestTroveData always returns up-to-date, post-accrual collateral value"
                    );
                } else {
                    gte(
                        troveData.entireDebt,
        ...
        for (uint256 i; i < listSize + 1; i++) {
            ...
            if (previousNodeId != 0) {
                ...
                gte(
                    ...
                    nextAnnualInterestRate,
                    "SR-02: Troves are sorted by interest rate in descending order"
                );
            }
    ...
    function property_BA01() public {
        eq(
            _after.ghostWeightedRecordedDebtAccumulator,
            _after.weightedRecordedDebtAccumulator,
            "BA-01: For all operations, weightedRecordedDebt is equal to the sum of trove debt * rate"
        );
    }
    ...
    function property_CS01() public {
        t(
            _before.collSurplusBalance >= _after.collSurplusBalance,
            "CS-01: Collateral surplus balances can increase only after a liquidation"
        );
    }

    function property_BT02() public {
        if (_before.batches[_getActor()].totalDebtShares == 0 && _after.batches[_getActor()].totalDebtShares == 0) {
            return;
        }
        ...
        if (_before.batches[_getActor()].totalDebtShares != 0) {
            ppfsBefore = _before.batches[_getActor()].debt / _before.batches[_getActor()].totalDebtShares;
        } else {
            ppfsBefore = 0;
        }

        if (_after.batches[_getActor()].totalDebtShares != 0) {
            ppfsAfter = _after.batches[_getActor()].debt / _after.batches[_getActor()].totalDebtShares;
        } else {
            ppfsAfter = 0;
        }
        ...
        if (_after.batches[_getActor()].debt != 0) {
            gte(
                ppfsAfter,
                ppfsBefore,
                "BT-02: Batch share PPFS always increases, unless the total debt in the batch is reset to 0"
            );
        }
    }
    ...
    function property_AP01() public {
        eq(
            ...
            "AP-01: newAggWeightedDebtSum == aggRecordedDebt if all troves have been synced in this block"
        );
    }
    ...
    function _sumBatchSharesAndDebt(address batchManager)
        internal
        returns (uint256 sumBatchDebt, uint256 sumbBatchShares)
    {
        uint256 trove = sortedTroves.getFirst();
